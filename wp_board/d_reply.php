...
// 인증 번호 //
$today_auth = date("Y:m:d");
$input_auth = Encrypt($today_auth);
...
// 정렬 들여쓰기에 필요한 thread 필드값 //
$query = "SELECT thread,right(thread,1) FROM wp_board WHERE fid = $fid AND length(thread) = length('$thread')+1 AND locate('$thread',thread) = 1 ORDER BY thread DESC LIMIT 1";
...
//비밀글 체크시 원본의 패스워드가 등록
if($secret){
	$que = "SELECT passwd FROM wp_board WHERE fid = '$fid' AND thread = '$thread'";
	$res  = mysql_query($que);
...
// 번호 체크 //
$query = "SELECT max(uid) FROM wp_board";
...
// TB 필드값 //
$values = array("$new_uid","$fid","$code","$writer","$user_pw","$email","$subject","$comment","$new_thread","$html",0,$signdate,"$id","$ip","$wkind","$secret");
