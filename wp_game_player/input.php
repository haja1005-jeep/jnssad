...
{
	// 1. 시군별 출전선수는 성별 관계없이 14까지만 신청 (2016년 03월 12일 LJS : 14명으로 수정)//
	unset($que);
	unset($res);
	unset($obj2);
	$que = "SELECT count(*) cnt FROM wp_game_player WHERE gam_uid = '$gam_uid' AND clu_code = '$obj->clu_code' AND spo_code = '$spo_code' AND spo_sec_code = '$spo_sec_code'";
	...
	if($spo_code_detail[0] == "LB201") //단체전 남녀구분없이 2명
	{
		unset($que);
		unset($res);
		unset($obj2);
...
//축구
if($spo_code == "FOOTSAL"){
	//1. 시군별 출전선수는 5명 2팀신청가능 (예비포함 14명)
	unset($que);
	unset($res);
	unset($obj2);
	$que = "SELECT count(*) cnt FROM wp_game_player
...
// 페이지 이동 //
echo ("<meta http-equiv='Refresh' content='0; URL=index.html?keyfield=pla_name&key=$obj->name'>");
?>