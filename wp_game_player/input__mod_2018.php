...
//{
	...
	{
		echo " where gam_uid='$gam_uid' AND pla_id='$player' AND spo_sec_code in ('PL100','PL200','PL300') AND spo_code_detail = '$spo_code_detail'";
		//exit;
		if($spo_sec_code == "PL100")
		{
			$Mysql->Delete(wp_game_player," where gam_uid='$gam_uid' AND pla_id='$player' AND spo_sec_code in ('PL100','PL200','PL300') AND spo_code_detail = '$spo_code_detail'");
		}
		else
		{
			$Mysql->Delete(wp_game_player," where gam_uid='$gam_uid' AND pla_id='$player' AND spo_sec_code in ('PL400','PL500','PL600') AND spo_code_detail = '$spo_code_detail'");
		}
	}
	else
	{
		$Mysql->Delete(wp_game_player," where gam_uid='$gam_uid' AND pla_id='$player'");
	}
//}
...
if($spo_code == "BILLIARDS")
{
	...
	*/
	...
	foreach($spo_code_detail as $kesy => $vals)
	{
        ...
        //세부종목 2인까지 출전
		$que = "SELECT count(*) cnt FROM wp_game_player WHERE gam_uid = '$gam_uid' AND clu_code = '$obj->clu_code' AND spo_code = '$spo_code' AND spo_sec_code = '$spo_sec_code' AND sex = '$sex' AND  tro_level_code = '$tro_level_code' AND spo_code_detail = '$vals'";
		$res = mysql_query($que);
		if(!$res)
		{
			ERROR("QUERY_ERROR");
...
/**** 여기까지 ************************************************************************************************************ /
	...
	unset($res);
	unset($obj2);
	$que = "SELECT count(*) cnt FROM wp_game_player WHERE gam_uid = '$gam_uid' AND clu_code = '$obj->clu_code' AND spo_code = '$spo_code' AND spo_sec_code = '$spo_sec_code' AND tro_level_code = '$tro_level_code' AND spo_code_detail = '$spo_code_detail'";
	$res = mysql_query($que);
	if(!$res)
	{
		ERROR("QUERY_ERROR");
...
{
	// 감독,코치 트레이너 ,주무 외 선수는 12명 이내에서 참가신청가능
	unset($que);
	unset($res);
	unset($obj2);
