...
if(is_array($test_game)){
	foreach($test_game as $keys => $vals){
		if(!in_array($vals,$ok_match)){ //성립이 체크되어있지 않으면 등록
			...
			$tro_level_code = getTrouble_level_code($spo_code, $tro_level_code);
			$game_name .= $tro_level_code." ".$wp_sex_code[$sex]." / ". $wp_sports_detail_code[$spo_code_detail];
...
echo ("
	<script>
		alert('기초데이터가 생성되었습니다.');
	</script>
	<meta http-equiv='Refresh' content='0; URL=index.html?$get_basic$link_get_add&mode=serial_basic&spo_code=$spo_code'>
");
?>