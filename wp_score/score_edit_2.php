...
if(!$spo_code)
{
	foreach($wp_club_code as $ckeys => $cvals)
	{
		...
		// TB 필드값 //
		$values = array("$s_gam_uid","$ckeys","참가점수","$a_score","참가자점수","$signdate");
		// DB 저장 //
...
if($spo_code)
{
	$score_array[$spo_code] = $wp_sports_code[$spo_code];
}
else
{
	$score_array = $wp_sports_code;
}

// 경기종목 검색
foreach($score_array as $keys => $values){
	...
	$game_que = "SELECT * FROM wp_game_serial WHERE gam_uid = '$s_gam_uid' AND spo_code = '$keys' AND test_game <> '1' ORDER BY game_code ASC";
	...
	while($game_obj = mysql_fetch_object($game_res))
	{
		...
		*/
		...
		// 경기에 참가하는 인원수를 구한다.
		if(in_array($keys, $wp_score_game)) // 기록경기이면
		{
			//역도경기는 별도관리
		...
		else if(in_array($keys, $wp_tot_game)) // 토너먼트경기이면
		{
		...
		else if(in_array($keys, $wp_league_game)) // 리그전경기이면
		{
		}
...
//		echo "<br>경기에 참가하는 인원 : $score_count ";
...
//			print_r($score_tb);
		...
		if(in_array($keys, $wp_tot_game))// 토너먼트경기이면
		{
...
//		echo "<BR>".$player_que;
		...
		while($player_obj = mysql_fetch_object($player_res))
		{
			...
			if(in_array($keys, $wp_tot_game) && (!$player_obj->rank_tot || $player_obj->rank_tot == 0))
			{
...
//			echo "<br> 계산식 $rank_score "."*".$real_city_count."/ ( ".$all_city." * 0.01 ) ";
			...
			if(array_key_exists($keys, $wp_life_game)) // 시범종목은 절반의 점수만 부여한다.
			{
...
//			echo "<br>시군 :[$player_obj->uid] $player_obj->clu_code , $player_obj->name 득점: $total_score <br>";
			...
			// TB 필드 //
			$fields = array("gam_uid","clu_code","spo_code","game_code","game_kind","pla_id","pla_name","rank","score","note","signdate");
			// TB 필드값 //
			$values = array("$s_gam_uid","$player_obj->clu_code","$player_obj->spo_code","$game_obj->game_code","$game_obj->game_kind","$player_obj->id","$player_obj->names","$player_obj->rank_tot","$total_score","$game_obj->game_code / {$player_obj->rank_tot}위 점수","$signdate");
			// DB 저장 //
			$insert = $Mysql->Insert(wp_score,$fields,$values);
...
if(!$spo_code)
{
	// 3. 개최지 출전 종목 중 종합득점에 가산점(20%)를 부여한다.
	...
	$fields = array("gam_uid","clu_code","spo_code","score","note","signdate");
	// TB 필드값 //
	$values = array("$s_gam_uid","$game_city_code","가산점","$add_score","주최시도 가산점 20%","$signdate");
	// DB 저장 //
	$insert = $Mysql->Insert(wp_score,$fields,$values);
}
//exit;
...
//페이지이동
echo ("<meta http-equiv='Refresh' content='0; URL=index.html?s_gam_uid=$s_gam_uid&mode=score&spo_code=$spo_code'>");
?>
